...
def main():
    ...
    eta_list = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45]
    delta = 0.05 
    classifier_free_guidance_prob = 0.15 
    alpha_list = [1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65]
    classifier_free_guidance_weight_list = [-3.5, -3.0, -2.5, -2.0, -1.5] 
    
    
    seed_list = range(8)
    file_path = './tuning_results/tune_11/result/tuning_result_ant_eta.csv'
    
    if not os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            header = ['eta','delta', 'classifier_free_guidance_prob','alpha','classifier_free_guidance_weight', 'mean (100th)', 'std (100th)', 'mean (80th)', 'std (80th)', 'mean (50th)', 'std (50th)']
    ...
    for eta in eta_list:  
        for alpha in alpha_list: 
            for classifier_free_guidance_weight in classifier_free_guidance_weight_list: 
                if eta in [0.0, 0.05, 0.1, 0.15, 0.25] and alpha not in [0.75, 0.65]: 
                    continue 
                ...
                for seed in seed_list:    
                    cmd = f"grep -Rlw './results/tune_6/AntMorphology-Exact-v0' -e 'noise: {noise}'  | xargs grep -lw 'classifier_free_guidance_prob: {classifier_free_guidance_prob}' | xargs grep -lw 'seed: {seed}'"
                ...
                with open(file_path, 'a') as file:
                    new_row = [eta,delta,classifier_free_guidance_prob,alpha, classifier_free_guidance_weight, mean_score_100th, std_score_100th, mean_score_80th, std_score_80th, mean_score_50th, std_score_50th]
                    writer = csv.writer(file)
