...
def main():
    ...
    df = pd.read_csv('./tuning_results/tune_19/result/tuning_result_ant.csv')
    df = df[df['mean (100th)']>=0.96]
    print(len(df))
    hyper_parameter_list= df[['eta','alpha','classifier_free_guidance_weight']].to_numpy()
    # hyper_parameter_list = [[0.05,0.65,-2.0],[0.05,0.7,-2.0]]
    seed_list = range(8)
    file_path = './tuning_results/tune_19/result/tuning_result_dkitty.csv'
        ...
        for delta in [0.25]: 
            for eta,alpha, classifier_free_guidance_weight in hyper_parameter_list: 
                ...
                for seed in seed_list:      
                    ...
                    cmd = f"grep -Rlw './results/tune_19/DKittyMorphology-Exact-v0/initial_lengthscale{lengthscale}/delta{delta}/seed{seed}' -e 'train: true'"
                ...
                with open(file_path, 'a') as file:
                    new_row = [lengthscale,delta, eta, alpha, classifier_free_guidance_weight, mean_score_100th, std_score_100th, mean_score_80th, std_score_80th, mean_score_50th, std_score_50th]
                    writer = csv.writer(file)
