...
def main():
    ...
    df1 = pd.read_csv('./tuning_results/tune_11/result/tuning_result_ant_eta.csv') 
    df2 = pd.read_csv('./tuning_results/tune_11/result/tuning_result_ant_eta_2.csv')
    df = pd.concat([df1,df2],ignore_index=True)
    df = df[df['classifier_free_guidance_weight']!=-1.0]
    df = df[df['mean (100th)']>=0.96]
    print(len(df))
    hyper_parameter_list= df[['eta','alpha','classifier_free_guidance_weight']].to_numpy()
    hyper_parameter_list = hyper_parameter_list[21:]
    seed_list = range(3)
    file_path = './tuning_results/tune_11/result/tuning_result_dkitty_eta.csv'
    
    if not os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            header = ['eta','classifier_free_guidance_prob','alpha','classifier_free_guidance_weight', 'mean (100th)', 'std (100th)', 'mean (80th)', 'std (80th)', 'mean (50th)', 'std (50th)']
    ...
    delta = 0.05 
    for eta, alpha, classifier_free_guidance_weight in hyper_parameter_list: 
        ...
        for seed in seed_list:   
            ...
            cmd = f"grep -Rlw './results/tune_6/DKittyMorphology-Exact-v0' -e 'noise: {noise}'  | xargs grep -lw 'classifier_free_guidance_prob: {classifier_free_guidance_prob}' | xargs grep -lw 'seed: {seed}'"
        ...
        with open(file_path, 'a') as file:
            new_row = [eta, classifier_free_guidance_prob, alpha, classifier_free_guidance_weight, mean_score_100th, std_score_100th, mean_score_80th, std_score_80th, mean_score_50th, std_score_50th]
            writer = csv.writer(file)
