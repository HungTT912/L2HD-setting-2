...
def main():
    ...
    seed_list = range(8)
    file_path = './tuning_results/tune_19/result/tuning_result_ant.csv'
    
    if not os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            header = ['sampling_lr','eta','alpha','classifier_free_guidance_weight', 'mean (100th)', 'std (100th)', 'mean (80th)', 'std (80th)', 'mean (50th)', 'std (50th)']
            writer = csv.writer(file)
            writer.writerow(header)
    best_tf8_params = pd.read_csv('./tuning_results/tune_19/result/tuning_result_tfbind8_lr0.05_3.csv')
    best_tf8_params = best_tf8_params[best_tf8_params['mean (100th)']>0.97]
    best_tf8_params = best_tf8_params[['eta','alpha','classifier_free_guidance_weight']].values.tolist()
    print(len(best_tf8_params))
    ...
    tested_parameters = df[['eta','alpha', 'classifier_free_guidance_weight']].values.tolist()
    sampling_lr = 0.001
    noise = 1e-2 
    # eta_list = [ 0., 0.05, 0.1, 0.15, 0.2, 0.25, 0.5, 0.75, 1.0]
    # alpha_list = [0.6, 0.65,0.7,0.75, 0.8, 0.85, 0.9, 0.95, 1.0, 1.05, 1.2, 1.5]
    # classifier_free_guidance_weight_list = [ -3.5, -3, -2.5, -2, -1.5]
    # for eta in eta_list:  
    #     for alpha in alpha_list: 
    #         for classifier_free_guidance_weight in classifier_free_guidance_weight_list: 
    for eta, alpha, classifier_free_guidance_weight in best_tf8_params: 
        if [eta,alpha, classifier_free_guidance_weight] in tested_parameters: 
            continue 
        ...
        for seed in seed_list:    
            cmd = f"grep -Rlw './results/tune_19/AntMorphology-Exact-v0/sampling_lr{sampling_lr}/noise{noise}/seed{seed}' -e 'seed: {seed}'"
        ...
        with open(file_path, 'a') as file:
            new_row = [sampling_lr,eta,alpha, classifier_free_guidance_weight, mean_score_100th, std_score_100th, mean_score_80th, std_score_80th, mean_score_50th, std_score_50th]
            writer = csv.writer(file)
