...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        # self.image_size = model_params.UNetParams.image_size 
        # self.layer_n = model_params.UNetParams.layer_n
        # self.kernel_size = model_params.UNetParams.kernel_size
        # self.depth = model_params.UNetParams.depth
    
        # self.denoise_fn = UNET_1D(input_dim=1,
    ...
    def p_sample_loop(self, x_low, y_low, y_high, clip_denoised=True, sample_mid_step=False, classifier_free_guidance_weight=0.):
        ...
        if sample_mid_step:
            ...
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
                img, x0_recon = self.p_sample(x_t=imgs[-1], x_low=x_low, y_low=y_low, y_high=y_high, i=i, clip_denoised=clip_denoised, classifier_free_guidance_weight=classifier_free_guidance_weight)
                imgs.append(img)
