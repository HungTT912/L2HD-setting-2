...
def main():
    ...
    seed_list = range(8)
    file_path = './tuning_results/tune_14/result/tuning_result_ant_2.csv'
    ...
    sampling_lr = 0.01
    noise = 1e-2 
    eta_list = [1.0, 0., 0.05, 0.1, 0.15, 0.2, 0.25, 0.5, 0.75]
    alpha_list = [1.8, 1.5, 2.0, 0.7, 0.8, 0.9, 0.95, 1.0, 1.2]
    classifier_free_guidance_weight_list = [ -3.5, -3, -2.5, -2, -1.5, 2,4]
    for eta in eta_list:  
        for alpha in alpha_list: 
            for classifier_free_guidance_weight in classifier_free_guidance_weight_list: 
                ...
                for seed in seed_list:    
                    cmd = f"grep -Rlw './results/tune_14/AntMorphology-Exact-v0/sampling_lr{sampling_lr}/noise{noise}/seed{seed}' -e 'seed: {seed}'"
