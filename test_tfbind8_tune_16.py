...
def main():
    ...
    seed_list = range(8)
    file_path = './tuning_results/tune_16/result/tuning_result_tfbind8.csv'
    ...
    tested_parameters = df[['sampling_lr','eta','alpha','classifier_free_guidance_weight']].values.tolist()
    ...
    for sampling_lr in [0.01]:
        for noise in [1e-2]: 
            for eta in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.5, 1.0]:
                for classifier_free_guidance_weight in [-4.0, -3.5, -3, -2.5, -2, -1.5, 0.0, 1.0, 2.0, 3.0, 4.0, 0.5, 1.5, 2.5, 3.5, 4.5, 5.0, 5.5, 6.0]: 
                    for alpha in [0.7, 0.8, 0.9, 1.0, 1.2, 1.5, 2., 1.6, 1.3, 1.1, 1.8]: 
                        if [sampling_lr, eta, alpha, classifier_free_guidance_weight] in tested_parameters: 
                            continue 
                        print([sampling_lr, eta, alpha, classifier_free_guidance_weight])
                        results_100th = []
                        results_80th = []
                        results_50th = []
                        ...
                        for seed in seed_list:   
                            ...
                            cmd = f"grep -Rlw './results/tune_16/TFBind8-Exact-v0/sampling_lr{sampling_lr}/noise{noise}/seed{seed}' -e 'train: true'"
