...
def main():
    ...
    file_path = './tuning_results/tune_6/result/tuning_result_tfbind10.csv'
    if not os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            header = ['classifier_free_guidance_prob','alpha','classifier_free_guidance_weight', 'mean (100th)', 'std (100th)', 'mean (80th)', 'std (80th)', 'mean (50th)', 'std (50th)']
            writer = csv.writer(file)
            writer.writerow(header)
    
    for sampling_lr in [0.01,0.1, 0.05]: 
